...
**Used in:** `ConfigManagerBepinex.cs` - Getting attacker/defender types for damage hooks
...
**Used in:** `ConfigManagerBepinex.cs` - Applying health multipliers
...
**Used in:** `ConfigManagerBepinex.cs` - Setting health after multiplier
...
**Used in:** `ConfigManagerBepinex.cs` - Getting base damage for multiplier calculation
...
**Used in:** `ConfigManagerBepinex.cs` - Applying `UnitMeleeDamageTakenMultiplier`
...
**Used in:** `ConfigManagerBepinex.cs` - Applying `UnitHealthMultiplier` to newly created units
...
**Used in:** `ConfigManagerBepinex.cs` - Applying `StructureDamageTakenMultiplier`

#### OnBuildingSpawn
Fired when a building is spawned. **Post** phase provides the building ID.
...
BuildingR3EventHooks.OnBuildingSpawn.Observable
    ...
    .Subscribe(args =>
    {
        // args.BuildingType - type of building
        // args.ReturnValue - building ID (Post phase only)
        
        int buildingId = (int)args.ReturnValue;
        eStructs buildingType = args.BuildingType;
        
        // Apply modifications to newly created building
    });
```
