2.7.0 - Multiplayer: host config sync (2026-09-24):
       ...
       - **Your own config files are never changed.** The host's files are stored in `BepInEx\config\CrusaderDETweaker\HostSync\` and read from there; the multiplier values are applied in memory without saving the .cfg file. When you leave the lobby, the match ends, or the host turns sync off, your own configs apply again without a restart: the unit/building tables are reset to the game's values and your files are re-applied, so no value from the host is left behind. (GameplaySettings is session state: keys your own file leaves at "no override" may keep the host's value until you restart the game; the log says so.)
       - Only the lobby owner's values are accepted (the Script Extender checks the Steam sender); the package is checked for size, format version, SHA-256 hash and a fixed list of files before anything is written. The host and the players must run the same major.minor version (2.7.x); otherwise everyone keeps their own configs and the tab says why.
       ...
       - **Fix: `RangedDamageTakenMultiplier` is no longer applied twice.** The ranged damage matrix multiplied its values by it when applied, and the multiplier is also applied when a projectile hits. At launch that never happened (the multipliers load after the matrices), but since 2.6.7 re-applies the matrices after every map-unload reset, a multiplier other than 1 was squared in play. The matrix now writes its values unscaled.
       - **Fix: the fire, Bedouin-heal and wall-cost multipliers stay applied.** They scale the game tables once at launch, and the Script Extender's map-unload reset (see 2.6.7) undid that; 2.6.7 re-applied only the files. The multipliers are now re-applied with them, after the tables are first reset to the game's values so a multiplier is never applied twice.
       - Developer: `ConfigLoader.ReapplyTemplateConfigs` is now the ONE path that writes the game tables: at launch (after the multipliers are bound; the config systems' `Load()` and the wall-cost / fire-heal `Apply()` no longer write them), after every SE unload reset, before every session start, and for the host sync. It now resets every table cell the mod wrote to the game's value (`TemplateBaseline.RestoreAll`) before applying, and lets the host sync switch back to the player's own files first when they are no longer a multiplayer client.
       ...
       - Developer: `Config/Sync/` (codec, manager, lobby ViewModel), `Config/Core/TemplateBaseline.cs` (every template write records the game's value first), `ConfigPaths.UseHostSyncFiles` redirect, new on-load test suite `ConfigSync` (the log now reads `ALL 5 TEST SUITES PASSED`), `Override/ScriptExtenderUI/CDTLobbySettings.xaml` shipped. Design: `docs/HOST_SYNC_DESIGN.md`.

2.6.8 - Settings stay applied in play, multipliers applied exactly once (2026-09-24):

       - **2.6.7 was never released; 2.6.8 replaces it.** 2.6.7 (below) fixed unit / structure / damage settings being reset to vanilla by the Script Extender on every map unload, by re-applying them after each reset. Re-applying exposed two bugs before 2.6.7 was published, so it was withdrawn: 2.6.8 = the 2.6.7 fix plus the two fixes below. Update straight from 2.6.6.
       - **Fix: `RangedDamageTakenMultiplier` was applied twice with 2.6.7.** The ranged damage matrix multiplied its values by it when applied, and the multiplier is also applied when a projectile hits. At launch that never happened (the multipliers load after the matrices), but every 2.6.7 re-apply did it, so any value other than 1 was squared in play. The matrix now writes its values unscaled; the hit-time multiplier is the only one.
       - **Fix: the fire, Bedouin-heal and wall-cost multipliers stay applied.** They scale the game tables once at launch; the Script Extender's map-unload reset undid that, and 2.6.7 re-applied only the config files. They are now re-applied together with the files, after every reset and before every game start.
       - **Every re-apply now gives exactly the launch result.** Before applying, the mod first puts every value it has ever written back to the game's own value (remembered the first time it writes it), then applies the files and the multipliers. Without that, a re-apply with no reset in between would scale the multipliers twice, and values the Script Extender does not reset (wall costs, ballista damage, run-speed bonuses) could keep a stale value. Launch, every map-unload reset and every game start all go through this one path (`ConfigLoader.ReapplyTemplateConfigs`).
       - Log: `[TemplateConfig] Applying unit / structure / damage-matrix settings (<reason>)` then `[TemplateConfig] Done (<reason>): N cells were reset to game values first`, at launch, after every reset and at every game start.
       - Maintenance: `scripts/deploy.ps1` now mirrors the build. A file in the game's plugin folder that the deployed build does not ship (for example an `Override` file left by a newer test build) is moved into the deploy backup, never deleted, and listed in the output. The Script Extender's `LobbyModSettings` folder (lobby-tab settings it stores in the plugin folder) is kept.
       - Developer: `Config/Core/TemplateBaseline.cs` (first-write baseline, restore-then-apply), matrix loaders report the cell they write, new on-load test suite `TemplateBaseline` (applying N times, with or without resets in between, equals applying once, multipliers included; the log now reads `ALL 5 TEST SUITES PASSED`).

2.6.7 - Unit, structure and damage settings now stay applied in play (UNRELEASED, superseded by 2.6.8; 2026-09-24):

       - **Fix: unit and structure stats (Speed, Health, GoldCost, ShieldHealth, run-speed bonuses, structure health / housing / costs) and all seven damage-matrix CSVs could be back to vanilla by the time you played.** The mod applied them once at game launch. SHCDE-SE restores every one of these game tables to its original values on each map unload (`GameUnitManagerAPI` / `GameBuildingManagerAPI` `OnUnloadMap` -> `ClearOverrides()` on every stat array, since SE 1.14.0), and the game raises map unloads while you only move through the menus (4 unloads logged on 2026-09-24 before any match started). They are now re-applied right after every such reset (`OnUnloadMap` Post) and before every session start (`OnStartMap` / `OnLoadSave` Pre, so starting units are created with them). The GameplaySettings file was not affected: it was already re-applied at each session start. Reported on Nexus ("the Speed of any unit I change doesn't do anything").
       - New log line `[TemplateConfig] Re-applying unit / structure / damage-matrix settings (<reason>)` followed by the usual `Applied unit configs` / `Applied structure configs` / `Damage matrices loaded` lines, at every reset and session start.
2.6.6 - Ported to Script Extender 2.8.0 (2026-09-22):

       - **Rebuilt for SHCDE-SE 2.8.0. This build requires SE 2.8.0 or newer, and 2.6.5 and earlier do not work on SE 2.x.** SE 2.0.0 removed its `Zhuqiaomon` memory library (replaced by `RedBird`) and changed the `LibraryLoaded` callback signature. The SE immediates this mod writes (siege-engine stones, ballista damage, run-speed bonuses, disease tiers, stealth and gate globals) are now `RedBird.X64.Assembly.Stateful` types, and the project references `RedBird.X64` / `RedBird.Core` / `RedBird.Abstractions` instead of `Zhuqiaomon.dll`. SE also renamed two wall tile flags (`CrenelationComponent`/`CrenelationModifier` -> `CrenelatedLow`/`CrenelatedHigh`, same bits), which the wall-cost detector uses.
       - **Minimum SE version is enforced**: `info.json` declares `Dependencies: 000shcdese >= 2.8.0` (SE 2.7+ reports an unmet range before startup) and the BepInEx dependency carries the same floor, so an older SE logs a clear "requires 2.8.0" refusal instead of a type-load error mid-game.
...
2.6.5 - Gameplay settings now apply when loading a save; syntax errors are called out (2026-09-16):
       ...
       - **Fix: the GameplaySettings file (siege-engine stones, trade prices, stealth, gameplay options, auto-trade) now also applies when a saved game is loaded.** It was re-applied only from the new-game / map-load hooks (`OnStartMap`, `OnLoadMap`). The Script Extender raises a separate `OnLoadSave` event for `DLL_LoadSaveGame` (its own Lua runtime treats a save load as a distinct session start, `LuaManager.OnLoadSave` vs `OnStartMap`), so a session resumed from a save never received any of these settings. `ConfigLoader.RegisterSessionHooks` now subscribes to `OnLoadSave` (Post, editor loads excluded). Reported on Nexus (weapon trade prices and siege-engine stone amounts "stay vanilla"). Whether `OnStartMap` also fires on a save load is unverified in-game; the extra subscription is harmless either way because `ApplyAllGlobalConfigs` is idempotent and already runs several times per session.
       - **A TOML syntax error in the GameplaySettings file is now reported as a plain-English ERROR** (`SYNTAX ERROR in ...: NOTHING in this file is applied ...`, with the parser's line,column diagnostics) both at launch and at every session start, instead of a generic warning at launch and a raw exception dump at session start. One bad line, e.g. `BuyPrice = 52 default: 155` without the `#`, makes the whole file unparseable (verified against the shipped Tomlyn: `Unexpected token found 'default' while expecting token 'newline'`), so every setting in it silently stayed vanilla. The generated file header and the Trade Prices block now say that the `#` must be kept.
       - **Log evidence for trade prices**: each applied good logs `buy A -> B, sell C -> D (read back B/D)`; the read-back dereferences the game's price table, so a mismatch is logged as a warning. The Siege Engines section logs the three stone values it wrote (one line per session start; not a read-back, the SE immediate returns its own cache).
       - Docs: CONFIGURATION_GUIDE (comment-syntax note; "re-applied at every session start, including a loaded save"), DEVELOPER_NOTES, CLAUDE.md, NEXUS_DESCRIPTION. Built against SHCDE-SE 1.44.0, same requirement as 2.6.2.

2.6.4 - Out-of-range stat values are clamped instead of silently reset (2026-09-08):

       - **Fix: a `ShieldHealth` (or other 16-bit stat) set above the game's maximum no longer reverts to `-1` on the next launch.** The Demolisher's shield health is a 16-bit game field (default 40000, maximum 65535). A TOML value above that failed the numeric conversion, and the per-launch config migration treated the unparseable line as a fresh entry and rewrote it as the `-1` sentinel with no warning, so the edit vanished every game start. Values above a property's maximum are now clamped to that maximum and written back as such, with a warning naming the property; values below the minimum (a negative number for an unsigned stat) are rejected with a warning instead of silently. Applies to every integer stat (`Health`, `ShieldHealth`, run-speed bonuses, structure health / housing). Reported on Nexus. Values inside the range were never affected.
       - Docs: the Units file header and CONFIGURATION_GUIDE state the 65535 ceiling for `ShieldHealth`.

2.6.3 - Spreadsheet-safe damage CSV parsing (2026-09-07):

       - **Fix: a damage-matrix CSV that went through a spreadsheet no longer gets its header wiped on launch.** Excel / Google Sheets save the blank line under the comment block as a line of commas; the parser took that line as the header row, the real header (`Arrow, Bolt, Slinger, Javelin`) became a data row of `-1`s, and the per-launch in-place reformat wrote that back. From then on every value was skipped (`Could not parse header '' as ProjectileType`, `Applied=0, Skipped=324`). The parser (`CsvHelper.ParseMatrix`, now pure and covered by a new on-load `CsvHelper` test suite, 8 tests) ignores comma-only lines, a leading UTF-8 BOM and trailing empty header cells, skips rows with an empty defender name, and a file whose header still cannot be parsed is left untouched with a clear error instead of being rewritten. Reported on Nexus. Repairing an already-broken file: put the attacker names back on the header line and delete the `-1` row, or delete the CSV to regenerate it.
       - The on-load QA self-check now runs 4 suites (`ALL 4 TEST SUITES PASSED`).

2.6.2 - Gold-only recruits via "NONE", RequiresHorse for Arabian / Bedouin mercenaries (2026-09-07):

       - **New: `WeaponType = "NONE"` / `ArmorType = "NONE"` removes an equipment requirement**, so a European unit can be hired for gold only, like the Arabian mercenaries. Deleting the line never did this: a missing key means "no override" (the string equivalent of the `-1` sentinel), so the game default stayed in force and the line was re-added on the next launch. `""` and `"STORED_NULL"` are accepted as synonyms. Reported on Nexus (an Archer still cost a bow after the `WeaponType` line was removed).
       - **Fix: `RequiresHorse = true` now works for Arabian and Bedouin mercenaries** (Horse Archer, Camel Lancer, Heavy Camel, or any other Mercenary Post unit). The game enforces a horse cost only for the 7 Barracks units, through its EU good-cost table; SHCDE-SE exposes exactly those 7 rows and silently ignores writes outside them, so for a Mercenary Post unit the setting wrote nothing and only the after-the-fact stable link happened. The `MakeTroop` recruit hook (already used for `MaxCount`) now refuses the hire when the local player has no free stable horse slot, trims a batch / Ctrl "recruit max" request to the free slots, and the finished recruit is linked to that slot as before. Barracks units keep the game's own horse check. The Mercenary Post hover shows no horse icon (that UI is hard-wired to the Barracks). Reported on Nexus.
       - **Fix: stable linking scans all of the player's stables**, not only the first one found, and skips a unit the game already linked (natively hired Knights), so no unit can occupy two slots. The logic moved to `Config/Core/StableTracking.cs`, shared by the recruit gate and the spawn / transition link paths.
       - **Fix: a misspelled `WeaponType` / `ArmorType` no longer silently makes the unit free.** An unknown resource name is logged as a warning and the game's requirement is left untouched (it used to be written as "no resource").
       - Docs: the generated Units file header and CONFIGURATION_GUIDE explain `NONE` and the Mercenary Post horse gate. Built against SHCDE-SE 1.44.0 (which also fixes the memory-protection bug behind 2.6.1's gatehouse crash; the gatehouse writes stay disabled in this build). `info.json` now also lists game version 2.8.2, the current Steam build.
