2.7.0 - Multiplayer: host config sync (2026-09-24):
       ...
       - Fix (no change at launch): the ranged damage matrix no longer multiplies its values by `RangedDamageTakenMultiplier` when applied. That code never ran at launch and the multiplier is applied when a projectile hits; re-applying the matrix (as the host sync does) would have applied it twice.
