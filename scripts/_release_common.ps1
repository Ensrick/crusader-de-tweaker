...
$script:PayloadWhitelist = @(
    ...
    'README.txt'   # generated by scripts\make_readme.ps1 at build time; never hand-edited
)
# Build by-products that are never shipped (debug symbols, library XML docs).
$script:DevArtifactPatterns = @('*.pdb', '*.xml')

# User README generator (functions only; see scripts\make_readme.ps1).
. (Join-Path $PSScriptRoot 'make_readme.ps1')
...
function Assert-PluginPayload([string]$Dir, [string]$Version) {
    ...
    Test-UserReadme (Join-Path $Dir 'README.txt') $Version   # missing / empty / wrong version line -> throw
